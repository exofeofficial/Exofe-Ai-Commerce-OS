...
def send_image_message(
    ...
    *,
    ...
    """Send a standalone image message — `image` is either
    {"link": <public https url>} or {"id": <media id from upload_media>}."""
    payload_image = {**image}
    if caption:
        payload_image["caption"] = caption
    return _send(
        {"type": "image", "image": payload_image},
        to,
        phone_number_id=phone_number_id,
        access_token=access_token,
    )
