...
from app.models.admin import (
    ...
    UpdateSubmissionFeaturedRequest,
    UpdateSubmissionFeaturedResponse,
)
...
def _submission_row(row: dict) -> AdminSubmissionRow:
    return AdminSubmissionRow(
        ...
        developer_name=f"{row['first_name']} {row['last_name']}",
...
def list_submissions_admin(
    ...
    rows = admin_service.list_submissions(db, status=status_filter)
    return AdminSubmissionsResponse(submissions=[_submission_row(row) for row in rows])
...
def get_submission_admin(
    ...
    row = admin_service.get_submission(db, submission_id)
    base = _submission_row(row)
...
@router.patch("/submissions/{submission_id}/status", response_model=UpdateSubmissionStatusResponse)
def update_submission_status_admin(
    submission_id: str,
    body: UpdateSubmissionStatusRequest,
    db: Annotated[Session, Depends(get_db)],
    admin: Annotated[CurrentUser, Depends(require_admin)],
) -> UpdateSubmissionStatusResponse:
    updated = admin_service.update_submission_status(
        db, admin_id=admin.user_id, submission_id=submission_id, status=body.status
    )
    return UpdateSubmissionStatusResponse(id=str(updated["id"]), name=updated["name"], status=updated["status"])


# ── PATCH /admin/submissions/{id}/featured ────────────────────────────────────
@router.patch("/submissions/{submission_id}/featured", response_model=UpdateSubmissionFeaturedResponse)
def update_submission_featured_admin(
    submission_id: str,
    body: UpdateSubmissionFeaturedRequest,
    db: Annotated[Session, Depends(get_db)],
    admin: Annotated[CurrentUser, Depends(require_admin)],
) -> UpdateSubmissionFeaturedResponse:
    updated = admin_service.set_submission_featured(
        db, admin_id=admin.user_id, submission_id=submission_id, featured=body.featured
    )
    return UpdateSubmissionFeaturedResponse(id=str(updated["id"]), name=updated["name"], is_featured=updated["is_featured"])