...
def get_whatsapp_image_ref(db: Session, business: dict, product_id: str) -> dict | None:
...
def _show_product_at_index(
    ...
    whatsapp_client.send_button_message(
        to,
        body=format_product_message(product),
        buttons=buttons,
        header_image=get_whatsapp_image_ref(db, business, product["id"]),
        phone_number_id=business["whatsapp_phone_number_id"],
        access_token=business["whatsapp_access_token"],
    )
    customer_repository.set_browse_state(
