...
# None = unlimited. This is the WhatsApp-conversation quota specifically
# (see the module docstring above) — a distinct meter from the "AI
# Actions" figures quoted on the pricing page (src/lib/plans.ts
# aiActionsPerMonth), which cover AI Store Builder / Assistant usage more
# broadly. Only Grow's 500 came from an explicit business decision; the
# rest are interpolated placeholders and should be reviewed before launch.
...
def _get_limit(db: Session, business_id: str) -> int | None:
    ...
    return PLAN_AI_LIMITS.get(plan, 100)
