...
export type DeveloperSubmissionInput = {
  ...
  description: string;
  version: string;
  category: "Productivity" | "Marketing" | "Store design" | "Orders & shipping" | "Customer support" | "Other";
  demoUrl: string;
  ...
  // Only meaningful when pricingModel is "paid" — what a merchant would
...
export function getDeveloperSubmissions() {
  return request<{ submissions: DeveloperSubmission[] }>("/developers/submissions");
}
// Public theme-store listing (themes.exofe.com) — no auth required, and
// deliberately missing fields (description, packageUrl, supportEmail) that
// only the developer who owns the submission needs to see.
...
export function getPublicThemes() {
  return request<{ themes: PublicTheme[] }>("/developers/submissions/public/themes");
}
export type PublicThemeDetail = PublicTheme & {
  description: string;
  supportEmail: string;
};
...
export function submitDeveloperSubmission(id: string) {
  return request<DeveloperSubmission>(`/developers/submissions/${id}/submit`, { method: "POST" });
}
// multipart, not the shared request() helper (always JSON) — no object
// storage is wired up yet, so the backend base64-encodes the zip into the
// response url itself (see MAX_PACKAGE_BYTES in developer_submissions.py
// for the size cap: 6MB raw, comfortably under the app-wide 8MB request
// body limit once base64 inflates it).
export async function uploadDeveloperPackage(file: File): Promise<{ url: string; name: string; size: number }> {
  const token = getToken();
  const form = new FormData();
  form.append("file", file);

  const res = await fetch(`${API_BASE_URL}/developers/submissions/upload`, {
    method: "POST",
...
// Same pattern as uploadDeveloperPackage above, for the theme/app screenshot
// shown on themes.exofe.com (see MAX_IMAGE_BYTES in developer_submissions.py: 3MB raw).
export async function uploadDeveloperScreenshot(file: File): Promise<{ url: string; name: string; size: number }> {
  const token = getToken();
  const form = new FormData();
  form.append("file", file);

  const res = await fetch(`${API_BASE_URL}/developers/submissions/upload-image`, {
    method: "POST",
    headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    body: form,
  });
...
// Facebook and WhatsApp aren't wired up yet — Google uses googleAuth() below instead.
export function signUpWithProvider(_provider: "facebook" | "whatsapp"): Promise<never> {
  return Promise.reject(new ApiError("Social sign-up isn't set up yet.", 501));
}

// Same stub as signUpWithProvider, kept separate so the wording matches
// whichever page is asking ("sign-up" vs "log-in") if these ever diverge.
export function loginWithProvider(_provider: "facebook" | "whatsapp"): Promise<never> {
...
export function subscribeToPlan(plan: "launch" | "grow" | "scale" | "enterprise") {
...
export type AdminSubscription = {
  ...
  businessId: string;
  businessName: string | null;
  plan: "trial" | "free" | "launch" | "grow" | "scale" | "enterprise";
...
export function activateSubscription(businessId: string, payload: { plan: "launch" | "grow" | "scale" | "enterprise"; amount: number }) {
...
export type AdminSubmissionStatus = "submitted" | "approved" | "rejected";
...
// Expected backend contract: GET /admin/submissions?status=... -> 200 { submissions: AdminSubmission[] }
// Omitting status returns everything except drafts (a developer's own
// unsubmitted work-in-progress, never meant for admin eyes).
export function getAdminSubmissions(status?: AdminSubmissionStatus) {
  return adminRequest<{ submissions: AdminSubmission[] }>(`/admin/submissions${status ? `?status=${status}` : ""}`);
}
// Expected backend contract: GET /admin/submissions/{id} -> 200 { submission: AdminSubmissionDetail }
export function getAdminSubmission(id: string) {
  return adminRequest<{ submission: AdminSubmissionDetail }>(`/admin/submissions/${id}`);
}
// Expected backend contract: PATCH /admin/submissions/{id}/status { status } -> 200 { id, name, status }
// Only works on a submission currently awaiting review ("submitted").
export function updateAdminSubmissionStatus(id: string, status: "approved" | "rejected") {
  return adminRequest<{ id: string; name: string; status: string }>(`/admin/submissions/${id}/status`, {
    method: "PATCH",
    body: JSON.stringify({ status }),
  });
}
// Expected backend contract: PATCH /admin/submissions/{id}/featured { featured } -> 200 { id, name, isFeatured }
// Only works on an "approved" submission; capped at 3 featured at once
// (see MAX_FEATURED_SUBMISSIONS in admin_service.py) — a 4th request 400s.
export function updateAdminSubmissionFeatured(id: string, featured: boolean) {
  return adminRequest<{ id: string; name: string; isFeatured: boolean }>(`/admin/submissions/${id}/featured`, {
    method: "PATCH",
...
// The dashboard's onboarding assistant (Home tab) — separate from the
// customer-facing WhatsApp AI above, this one talks to the business owner.
...
// multipart, not the shared request() helper (always JSON) — the backend
// needs this shape either way now so it can also take an optional attached
// screenshot, passed straight through to Gemini alongside the question.
export async function askAssistant(message: string, image?: File): Promise<AssistantReply> {
  const token = getToken();
  const form = new FormData();
