...
function ActivatePlanControl({
  ...
  // Enterprise has no fixed catalog price (negotiated manually), so it's
  // left out of this quick-select — activate it from the deal itself with
  // whatever amount was agreed, not this fixed-price control.
  const ACTIVATABLE_PLANS = PLANS.filter((p) => p.monthly !== null);

  const [selectedPlan, setSelectedPlan] = useState<PlanId>(
    (subscription.plan === "trial" || subscription.plan === "free" || subscription.plan === "enterprise"
      ? "launch"
  ...
  const handleActivate = async () => {
    ...
    setError(null);
    try {
      const plan = ACTIVATABLE_PLANS.find((p) => p.id === selectedPlan)!;
